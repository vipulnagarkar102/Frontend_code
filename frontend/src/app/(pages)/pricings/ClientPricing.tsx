...
type RawApiResponse = {
  success: boolean;
  data: Record<string, PlanFromAPI[]>;
};
...
export default function ClientPricing({ initialAnnual, onToggle }: ClientPricingProps) {
  const [isAnnual, setIsAnnual] = useState(initialAnnual);
  const [plans, setPlans] = useState<Array<{ title: string; price: number; desc: string | string[] }>>([]);
  ...
  useEffect(() => {
    async function fetchPlans() {
      ...
      try {
        ...
        const flatPlans = Object.values(json.data)
          .flat()
          .map(plan => ({
